...
let doorModel = null;
let exitDoorMixer = null;
let exitDoorStaticAction = null;
let exitDoorOpenAction = null;
let exitDoorCloseAction = null;
let exitDoorUsesReverseClose = false;
let exitDoorState = "closed";

const DOOR_OPEN_DISTANCE = 3.6;
const DOOR_CLOSE_DISTANCE = 4.0;
...
function findClipByHints(animations, hints) {
  if (!animations || !animations.length) return null;
  for (const clip of animations) {
    const name = (clip.name || "").toLowerCase();
    if (hints.every((h) => name.includes(h))) return clip;
  }
  return null;
}

function normalizeDoorModel(model) {
  const box = new THREE.Box3().setFromObject(model);
  if (box.min.y === Infinity || box.max.y === -Infinity) return;
  const size = new THREE.Vector3();
  box.getSize(size);
  if (size.y > 0) {
    const s = DOOR_TARGET_HEIGHT / size.y;
...
function clearExitDoorState() {
  if (exitDoorMixer) {
    exitDoorMixer.stopAllAction();
  }
  exitDoorMixer = null;
  exitDoorStaticAction = null;
  exitDoorOpenAction = null;
  exitDoorCloseAction = null;
  exitDoorUsesReverseClose = false;
  exitDoorState = "closed";
}

function playDoorStaticPose() {
  if (!exitDoorStaticAction) return;
  exitDoorStaticAction.reset();
  exitDoorStaticAction.setLoop(THREE.LoopRepeat, Infinity);
  exitDoorStaticAction.clampWhenFinished = false;
  exitDoorStaticAction.timeScale = 1;
  exitDoorStaticAction.enabled = true;
  exitDoorStaticAction.play();
}

function playDoorTransition(openDoor) {
  if (!exitDoorOpenAction && !exitDoorCloseAction) return;

  if (exitDoorStaticAction) {
    exitDoorStaticAction.stop();
  }

  if (openDoor) {
    const action = exitDoorOpenAction;
    if (!action) return;
    ...
    action.paused = false;
    action.timeScale = 1;
    if (action.time >= clipDuration - 0.001) {
      action.time = 0;
    }
    action.play();
  } else {
    if (exitDoorUsesReverseClose) {
      const action = exitDoorOpenAction;
      if (!action) return;
      const clipDuration = action.getClip().duration;
      action.enabled = true;
      ...
      const action = exitDoorCloseAction;
      if (!action) return;
      action.enabled = true;
      action.setLoop(THREE.LoopOnce, 1);
      action.clampWhenFinished = true;
...
function setExitDoorOpen(openDoor) {
  const nextState = openDoor ? "open" : "closed";
  if (exitDoorState === nextState) return;
  exitDoorState = nextState;

  if (exitDoorOpenAction || exitDoorCloseAction) {
    playDoorTransition(openDoor);
    return;
  }
...
function createExitDoorAt(cellWorldPos) {
  clearExitDoorState();

  if (!doorModel || !doorModel.scene) return null;
  const doorRoot = doorModel.scene.clone(true);
  normalizeDoorModel(doorRoot);
  doorRoot.position.set(cellWorldPos.x, 0, cellWorldPos.z);

  const hasAnimations = doorModel.animations && doorModel.animations.length > 0;
  if (hasAnimations) {
    exitDoorMixer = new THREE.AnimationMixer(doorRoot);
    const clips = doorModel.animations;

    const staticClip =
      findClipByHints(clips, ["static", "pose"]) ||
      findClipByHints(clips, ["static"]) ||
      findClipByHints(clips, ["pose"]);

    const comboClip =
      findClipByHints(clips, ["close", "open"]) ||
      findClipByHints(clips, ["open", "close"]);
    const openClip = findClipByHints(clips, ["open"]);
    const closeClip = findClipByHints(clips, ["close"]);

    if (staticClip) {
      exitDoorStaticAction = exitDoorMixer.clipAction(staticClip);
      playDoorStaticPose();
    }

    if (comboClip) {
      exitDoorOpenAction = exitDoorMixer.clipAction(comboClip);
      exitDoorCloseAction = exitDoorOpenAction;
      exitDoorUsesReverseClose = true;
    } else {
      if (openClip) exitDoorOpenAction = exitDoorMixer.clipAction(openClip);
      if (closeClip) exitDoorCloseAction = exitDoorMixer.clipAction(closeClip);
      exitDoorUsesReverseClose = !exitDoorCloseAction && !!exitDoorOpenAction;
      if (!exitDoorCloseAction && exitDoorOpenAction) {
        exitDoorCloseAction = exitDoorOpenAction;
      }
    }

    if (exitDoorOpenAction) {
      exitDoorOpenAction.clampWhenFinished = true;
      exitDoorOpenAction.enabled = true;
      exitDoorOpenAction.time = 0;
      exitDoorOpenAction.paused = true;
      exitDoorOpenAction.play();
      exitDoorOpenAction.paused = true;
    }

    if (exitDoorCloseAction && exitDoorCloseAction !== exitDoorOpenAction) {
      exitDoorCloseAction.clampWhenFinished = true;
      exitDoorCloseAction.enabled = true;
      exitDoorCloseAction.time = 0;
      exitDoorCloseAction.paused = true;
      exitDoorCloseAction.play();
      exitDoorCloseAction.paused = true;
    }
  }
...
function setupLevel(levelNum) {
  ...
  if (exitMesh) {
    ...
    if (exitMesh.userData && exitMesh.userData._isFallbackExitBox) {
  ...
  exitMesh = createExitDoorAt(exitCell);
  if (!exitMesh) {
    ...
    exitMesh.userData._isFallbackExitBox = true;
...
function attemptMove(delta) {
  ...
  if (exitMesh) {
    if (distToExit <= DOOR_OPEN_DISTANCE) {
      setExitDoorOpen(true);
    } else if (distToExit > DOOR_CLOSE_DISTANCE) {
...
gltfLoader.load(
  ...
  (gltf) => {
    doorModel = gltf;
    if (exitMesh && exitMesh.userData && exitMesh.userData._isFallbackExitBox && exitCell) {
      scene.remove(exitMesh);
      exitMesh.geometry?.dispose();
      exitMesh.material?.dispose();
      exitMesh = createExitDoorAt(exitCell);
  ...
  undefined,
  (err) => console.error("door_wood.glb load error:", err)
);